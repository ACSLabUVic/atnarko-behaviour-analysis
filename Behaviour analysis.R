...
setwd("C:\\Users\\cdret\\Dropbox\\Atnarko\\Chapter 4 - Behaviour\\Clean and annotated code\\Submit")
...
names(frequency_df) <- c("Focal sample durations (min)", "Frequency")

ggplot(data = frequency_df, aes(x = `Focal sample durations (min)`, y = Frequency)) +
...
# 'Unobservable' occurances 
# How long were bears unobservable for (summary of min, max, prop)
proportion_summary <- data %>%
  group_by(focalbear_event) %>%  # Group by focalbear_event
  summarise(
    total_duration = sum(duration_in_seconds, na.rm = TRUE),  # Total duration in seconds for the group
    duration_coarse_code_0 = sum(duration_in_seconds[coarse_code == 0], na.rm = TRUE),  # Duration where coarse_code is 0
    proportion_coarse_code_0 = duration_coarse_code_0 / total_duration  # Proportion
  ) %>%
  summarise(
    min_proportion = min(proportion_coarse_code_0, na.rm = TRUE),
    max_proportion = max(proportion_coarse_code_0, na.rm = TRUE),
    mean_proportion = mean(proportion_coarse_code_0, na.rm = TRUE)
  )
...
# Salmon data
# Attribute weekly salmon count data from the Atnarko (same methods and salmon dataset as Field et el., 2024)
salm <- read.csv("salmon.csv")

# Rename columns
salm  <- salm  %>%
  rename(yearweek_ID = yearweek_id.x,
         River_section_ID = River_section_ID.x.x)
...
# Run this line of code if you want to know when the river was closed to boats, and Belarko was closed to people (Fisheries Pool was still open). 
# Create the closures column based on the conditions
# data$closures <- ifelse(
#   (data$date >= "2021-09-01" & data$date <= "2021-09-05" & format(as.POSIXct(data$start_time_of_focal, format="%H:%M:%S"), "%H:%M:%S") > "13:30:00") |
#     (data$date >= "2021-09-06" & data$date <= "2021-09-10" & format(as.POSIXct(data$start_time_of_focal, format="%H:%M:%S"), "%H:%M:%S") < "13:30:00"),
#   "river closed",
#   "river open"
# )
# length(unique(data$focalbear_event[data$closures=="river closed"]))
# length(unique(data$focalbear_event[data$closures=="river open"]))
...
# Create a column that indicates whether snorkeling or carry to cover occurred at all within a focalbear_event
data <- data %>%
  group_by(focalbear_event) %>%
  mutate(snorkel_present_in_sample = ifelse(any(snorkel == "y"), 1, 0))

data <- data %>%
  group_by(focalbear_event) %>%
  mutate(carry_present_in_sample = ifelse(any(carry_to_cover == "y"), 1, 0))
...
data$timestamp_focal.bhv.change <- ymd_hms(data$timestamp_focal.bhv.change)
...
scans <- scans %>%
  dplyr::rename(site_name = site_name.x,
                timestamp_scans = timestamp.s)
...
# Inappropriate to have NAs for bears, people, and boats (ie, there were always 0 or >0. Convert NA to 0's.
data<- data %>%
...
# write.csv(data, "C:\\Users\\cdret\\Dropbox\\Atnarko\\Chapter 4 - Behaviour\\test2.csv")

# Histogram to look at the number of cases within a focal sample. 
ggplot(data, aes(x = reorder(focalbear_event, focalbear_event, function(x) -length(x)))) +
  labs(x = "Focal samples",
       y = "Number of cases") +
...
hours2 <- hours2 %>%
  mutate(total_length_hours = total_length_sum / 3600)  
sum(hours2$total_length_hours)

# Redo below. Omit when done.
a <- 18.905833 -15.083611 # hours with males - hours without males in 2020
b <- 11.800000 -11.501944 # hours with males - hours without males in 2021

a + b # number of hours we observed males across the whole study.

# ggplot(mean_time_diff_per_group, aes(x = reorder(focalbear_event, mean_time_diff), y = mean_time_diff)) +
#   geom_col(colour = "black") +  # Scatter plot with blue points
#   theme_classic() +
#   labs(
#     x = "Focal samples",
#     y = "Mean time between scans (min)",
#     title = ""
#   ) +
#   theme(
#     axis.text.x = element_blank(),  # Remove x-axis text labels
...
# Omit focal samples where there were either only 1 scan, or there were scans >5 min apart
...
# How many focal samples were unknowns?
unknowns <- data %>%
  filter(bear_name == "UNKNOWN") %>%   
...
ggplot(humans_per_focal, aes(x = max_land_human_num)) +
  ...
  theme(axis.text.x = element_text(size = 12),
        axis.text.y = element_text(size = 12))
ggsave(file="C:\\Users\\cdret\\Dropbox\\Atnarko\\Chapter 4 - Behaviour\\Clean and annotated code\\Submit\\land.humans.per.observation.period.png", width=7, height=5, dpi=300)
...
ggplot(boats_per_focal, aes(x = max_boats)) +
  ...
  theme(axis.text.x = element_text(size = 12),
        axis.text.y = element_text(size = 12))
ggsave(file="C:\\Users\\cdret\\Dropbox\\Atnarko\\Chapter 4 - Behaviour\\Clean and annotated code\\Submit\\boats.per.observation.period.png", width=7, height=5, dpi=300)
...
ggplot(data, aes(x = land_human_num)) +
  ...
  theme(axis.text.x = element_text(size = 12),
        axis.text.y = element_text(size = 12))
ggsave(file="C:\\Users\\cdret\\Dropbox\\Atnarko\\Chapter 4 - Behaviour\\Clean and annotated code\\Submit\\land.humans.per.scan.png", width=7, height=5, dpi=300)
...
ggplot(data, aes(x = num_boats)) +
  ...
  theme(axis.text.x = element_text(size = 12),
        axis.text.y = element_text(size = 12))
ggsave(file="C:\\Users\\cdret\\Dropbox\\Atnarko\\Chapter 4 - Behaviour\\Clean and annotated code\\Submit\\boats.per.scan.png", width=7, height=5, dpi=300)
...
ggsave(file="Behaviour analysis results.png", width=12, height=6, dpi=300)
...
# ggsave(file="results2_second model.png", width=12, height=6, dpi=300)
